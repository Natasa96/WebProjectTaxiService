...
namespace TaxiService.Controllers
{
    public class DriverController : ApiController
    {
        ...
        [HttpPost, Route("api/Driver/Update")]
        public IHttpActionResult Update(Driver data)
        {
            lock (DB)
            {
                int id = DB.DriverDb.ToList().IndexOf(DB.DriverDb.ToList().Find(p => p.Username == data.Username));
                DB.DriverDb.ToList()[id].Password = data.Password;
                DB.DriverDb.ToList()[id].ContactPhone = data.ContactPhone;
                DB.DriverDb.ToList()[id].Email = data.Email;
                DB.DriverDb.ToList()[id].Firstname = data.Firstname;
                DB.DriverDb.ToList()[id].Lastname = data.Lastname;
                DB.DriverDb.ToList()[id].Gender = data.Gender;
                DB.DriverDb.ToList()[id].JMBG = data.JMBG;
                DB.SaveChanges();
                return Ok();
            }
        }
